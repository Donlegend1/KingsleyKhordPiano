...
const plans = [
  { id: 1, type: 'monthly', tier: 'standard', amount: { eur: 23, usd: 26, naira: 38000 }, img: '/icons/icon.png',bg:"" },
  { id: 2, type: 'monthly', tier: 'premium', amount: { eur: 40, usd: 45, naira: 70000 }, img: '/icons/price2.png', bg:"/images/Background.jpg" },
  { id: 3, type: 'yearly', tier: 'standard', amount: { eur: 189, usd: 215, naira: 320000 }, img: '/icons/icon.png', bg:"" },
  { id: 4, type: 'yearly', tier: 'premium', amount: { eur: 369, usd: 420, naira: 650000 }, img: '/icons/price2.png', bg:"/images/Background.jpg" },
];
...
const PlanSwitchAndCurrencySelect = () => {
  ...
  return (
    <section className="container mx-auto px-5 md:px-28 h-[100vh] bg-white mb-20" id="plans">
      <div className="text-center my-12 mx-auto">
        <p className="font-bold text-4xl">Gain Immediate Entry to </p>
        <p className="font-bold text-4xl my-3 text-[#BC1414]">Kingsley Khord Piano Academy</p>
      ...
      <div className="flex flex-col md:flex-row justify-between max-w-3xl mx-auto mb-8">
        <div className="relative flex gap-0">
          <button
            className={`px-4 py-2 rounded-full z-10 -mr-8 transition ${selectedPlan === 'monthly' ? 'bg-gray-300 text-gray-800' : 'bg-white text-gray-700 border'}`}
            onClick={() => handlePlanToggle('monthly')}
          >
            Monthly Plan
          </button>
          <button
            className={`px-4 py-2 rounded-full z-0 transition -ml-8 ${selectedPlan === 'yearly' ? 'bg-gray-300 text-gray-800' : 'bg-white text-gray-700 border'}`}
            onClick={() => handlePlanToggle('yearly')}
      >
            <span className='text-gray-700 ml-12'>
            Yearly Plan
            </span>
             <span className="text-red-600 mx-2 font-sf text-xs">Save 30%</span>
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-center gap-6">
        {filteredPlans.map(plan => (
         <div key={plan.id} className="bg-white border border-[#C2D3DD73] rounded-xl shadow-lg p-6 w-full md:w-1/3" style={{ backgroundImage: `url(${plan.bg})` }}>
            <div className="flex justify-center mb-4 relative">
              <img src={plan.bg} alt="" className="absolute top-0 left-0 w-full h-full object-cover opacity-50 rounded-xl" />
            </div>
            <img src={plan.img} alt="" className="mb-4 p-3 border h-16  rounded-3xl" />
            <h3 className="text-xl font-semibold text-black mb-4">{plan.tier}  Plan</h3>
            <p className="text-2xl font-bold mb-2">{currencySigns[currency]}{plan.amount[currency].toLocaleString()}</p>
          ...
          {plan.tier == "premium" &&
           (
          <ul className="text-sm text-gray-700 mb-6 list-disc list-inside">
              <li className='text-red-500 font-sf font-semibold'>Everyting in the standard plan</li>
  ...
  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 py-9">
    ...
    <div className="bg-[#2D2D2D] rounded-xl shadow-xl p-12 w-full max-w-lg mx-auto">
      ...
      <p className="text-center text-white mb-4">
        {currencySigns[currency]}{selectedPlanDetails.amount[currency].toLocaleString()} for {selectedPlanDetails.tier} Plan
      </p>
      ...
      <div className="mt-4 flex items-center justify-center text-sm text-gray-400">
        <p>
          ...
          <span className="inline-block mx-2 bg-gray-300 rounded-md p-1">
            <img src="/icons/stripe2.png" alt="Stripe" className="h-4" />
          </span>
          <span className="inline-block mx-2 bg-gray-300 rounded-md p-1">
            <img src="/icons/paystack2.png" alt="Paystack" className="h-4" />
          </span>
        </p>
      </div>
    </div>
