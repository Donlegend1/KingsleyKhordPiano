import React, { useEffect, useState } from "react";
import ReactDOM from "react-dom/client";
import axios from "axios";
import dayjs from "dayjs";
import duration from "dayjs/plugin/duration";
import relativeTime from "dayjs/plugin/relativeTime";
import isoWeek from "dayjs/plugin/isoWeek";

import {
    useFlashMessage,
    FlashMessageProvider,
} from "../Alert/FlashMessageContext";
import { calculateCountdown, formatLocalTime } from "@/utils/formatRelativeTime";

const LiveShowCard = () => {
    const [shows, setShows] = useState([]);
    const [countdowns, setCountdowns] = useState({});
    const { showMessage } = useFlashMessage();

    const authUser = window.authUser || {};
    const isPremium = authUser?.premium;
    dayjs.extend(duration);
    dayjs.extend(relativeTime);
    dayjs.extend(isoWeek);

    useEffect(() => {
        const fetchShows = async () => {
            try {
                const res = await axios.get("/api/live-shows", {
                    params: { filter: "future" }, // or "previous"
                });

                setShows(res.data);

                const countdownsObj = {};
                res.data.forEach((show) => {
                    countdownsObj[show.id] = calculateCountdown(
                        show.start_time
                    );
                });

                setCountdowns(countdownsObj);
            } catch (error) {
                console.error("Error fetching live shows:", error);
            }
        };

        fetchShows();
    }, []);

    useEffect(() => {
        const interval = setInterval(() => {
            setCountdowns((prev) => {
                const updated = { ...prev };
                shows.forEach((show) => {
                    updated[show.id] = calculateCountdown(show.start_time);
                });
                return updated;
            });
        }, 1000);

        return () => clearInterval(interval);
    }, [shows]);

    const handleRestrictedClick = (e) => {
        e.preventDefault();
        showMessage(
            "This content is available for premium users only.",
            "error"
        );
    };


    return (
        <section className="max-w-7xl mx-auto px-6 py-16">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {shows && shows.length > 0 ? (
                    shows.map((show) => {
                        const date = dayjs(show.start_time);
                        const isRestricted = show.access_type === "premium" && !isPremium;
                        const countdown = countdowns[show.id] || { days: 0, hours: 0, minutes: 0, seconds: 0 };
                        const { localDate, localTime, tzLabel } = formatLocalTime(show.start_time);
                        const isSession = show.category === "session";
                        const calendarUrl = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(show.title)}&dates=${date.format("YYYYMMDDTHHmmss")}/${date.add(1, "hour").format("YYYYMMDDTHHmmss")}&details=${encodeURIComponent(show.zoom_link || "")}`;

                        if (isSession) {
                            const bookingsCount = show.bookings_count ?? 0;
                            const maxSlots = show.max_slots ?? 0;
                            const isFull = maxSlots > 0 && bookingsCount >= maxSlots;
                            const alreadyBooked = !!show.booked_by_user;
                            const avatars = (show.bookings || []).slice(0, 3);
                            const overflow = Math.max(bookingsCount - avatars.length, 0);

                            return (
                                <div
                                    key={show.id}
                                    className="relative overflow-hidden rounded-2xl border border-white/10 shadow-xl p-7 flex flex-col transition-all duration-300 hover:shadow-2xl hover:-translate-y-0.5 bg-gradient-to-br from-gray-500 via-gray-800 to-black"
                                >
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className="w-10 h-10 rounded-xl bg-indigo-500/90 flex items-center justify-center flex-shrink-0 shadow-lg shadow-indigo-900/40">
                                            <i className="fa-solid fa-people-group text-white text-sm"></i>
                                        </div>
                                        <span className="text-indigo-300 text-[11px] font-bold uppercase tracking-widest bg-indigo-400/10 border border-indigo-400/30 px-2.5 py-1 rounded-full">
                                            Live Session
                                        </span>
                                    </div>

                                    <h3 className="text-xl font-bold text-white leading-tight mb-3">
                                        {show.title}
                                    </h3>

                                    <div className="flex items-center gap-4 text-gray-300 text-sm font-medium mb-6">
                                        <div className="flex items-center gap-1.5">
                                            <i className="fa-regular fa-calendar text-yellow-400"></i>
                                            {localDate}
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <i className="fa-regular fa-clock text-yellow-400"></i>
                                            {localTime} {tzLabel}
                                        </div>
                                    </div>

                                    <div className="flex items-center justify-between mb-6 bg-white/5 border border-white/10 rounded-xl px-4 py-3.5">
                                        <div className="flex items-center gap-2">
                                            <i className="fa-solid fa-people-group text-indigo-300 text-sm"></i>
                                            <div>
                                                <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide">Seats Filled</p>
                                                <p className="text-base font-bold text-white leading-none">
                                                    {bookingsCount} / {maxSlots}
                                                </p>
                                            </div>
                                        </div>
                                        {avatars.length > 0 && (
                                            <div className="flex items-center -space-x-2">
                                                {avatars.map((user) => (
                                                    <div key={user.id} className="w-7 h-7 rounded-full ring-2 ring-gray-900 overflow-hidden bg-indigo-500/30 border border-indigo-400/40 flex items-center justify-center">
                                                        {user.passport ? (
                                                            <img src={user.passport} alt={user.first_name} className="w-full h-full object-cover" />
                                                        ) : (
                                                            <span className="text-[10px] font-bold text-indigo-200">
                                                                {(user.first_name || "?").charAt(0).toUpperCase()}
                                                            </span>
                                                        )}
                                                    </div>
                                                ))}
                                                {overflow > 0 && (
                                                    <div className="w-7 h-7 rounded-full ring-2 ring-gray-900 bg-indigo-500 flex items-center justify-center text-[10px] font-bold text-white">
                                                        +{overflow}
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                    </div>

                                    <div className="flex flex-col gap-2.5 mt-auto">
                                        <a
                                            href={isRestricted ? "#" : `/member/live-session/${show.id}/confirm`}
                                            onClick={isRestricted ? handleRestrictedClick : undefined}
                                            aria-disabled={isFull || alreadyBooked}
                                            className={`w-full py-3.5 rounded-xl text-sm font-bold text-center uppercase tracking-widest transition-all duration-200 ${
                                                isFull || alreadyBooked
                                                    ? "bg-white/10 text-gray-400 cursor-not-allowed pointer-events-none"
                                                    : "bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-900/40"
                                            }`}
                                        >
                                            {alreadyBooked ? "Slot Booked" : isFull ? "Session Full" : "Join the Slot"}
                                        </a>
                                        <a
                                            href={isRestricted ? "#" : calendarUrl}
                                            onClick={isRestricted ? handleRestrictedClick : undefined}
                                            className="w-full py-3 rounded-xl border border-white/15 bg-white/5 text-white text-xs font-bold text-center uppercase tracking-widest hover:bg-white/10 transition-all duration-200"
                                        >
                                            <i className="fa-regular fa-calendar-plus mr-2"></i>
                                            Add to Calendar
                                        </a>
                                    </div>
                                </div>
                            );
                        }

                        return (
                            <div
                                key={show.id}
                                className="relative overflow-hidden rounded-2xl border border-white/10 shadow-xl p-7 flex flex-col transition-all duration-300 hover:shadow-2xl hover:-translate-y-0.5 bg-gradient-to-br from-gray-500 via-gray-800 to-black"
                            >
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-red-500/90 flex items-center justify-center flex-shrink-0 shadow-lg shadow-red-900/40">
                                            <i className="fa-solid fa-tower-broadcast text-white text-sm"></i>
                                        </div>
                                        <span className="text-yellow-300 text-[11px] font-bold uppercase tracking-widest bg-yellow-400/10 border border-yellow-400/30 px-2.5 py-1 rounded-full">
                                            Live Event
                                        </span>
                                    </div>
                                    {show.access_type === "premium" && (
                                        <div className="flex items-center gap-1 bg-amber-400/10 border border-amber-400/30 text-amber-300 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">
                                            <img src="/icons/diamondred.png" alt="Premium" className="w-3 h-3" />
                                            Premium
                                        </div>
                                    )}
                                </div>

                                <h3 className="text-xl font-bold text-white leading-tight mb-3">
                                    {show.title}
                                </h3>

                                <div className="flex items-center gap-4 text-gray-300 text-sm font-medium mb-6">
                                    <div className="flex items-center gap-1.5">
                                        <i className="fa-regular fa-calendar text-yellow-400"></i>
                                        {localDate}
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                        <i className="fa-regular fa-clock text-yellow-400"></i>
                                        {localTime} {tzLabel}
                                    </div>
                                </div>

                                <div className="grid grid-cols-4 gap-2 mb-6">
                                    {[
                                        { label: 'Days', val: countdown.days },
                                        { label: 'Hours', val: countdown.hours },
                                        { label: 'Min', val: countdown.minutes },
                                        { label: 'Sec', val: countdown.seconds }
                                    ].map((unit, i) => (
                                        <div key={i} className="flex flex-col items-center justify-center bg-white/5 rounded-xl py-2.5 border border-white/10">
                                            <span className="text-lg font-bold text-white tabular-nums">{unit.val ?? '-'}</span>
                                            <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wide">{unit.label}</span>
                                        </div>
                                    ))}
                                </div>

                                <div className="flex flex-col gap-2.5 mt-auto">
                                    <a
                                        href={isRestricted ? "#" : show.zoom_link}
                                        onClick={isRestricted ? handleRestrictedClick : undefined}
                                        className="w-full py-3.5 rounded-xl bg-red-500 text-white text-sm font-bold text-center uppercase tracking-widest hover:bg-red-600 shadow-lg shadow-red-900/40 transition-all duration-200"
                                    >
                                        Join Live Show
                                    </a>
                                    <a
                                        href={isRestricted ? "#" : calendarUrl}
                                        onClick={isRestricted ? handleRestrictedClick : undefined}
                                        className="w-full py-3 rounded-xl border border-white/15 bg-white/5 text-white text-xs font-bold text-center uppercase tracking-widest hover:bg-white/10 transition-all duration-200"
                                    >
                                        <i className="fa-regular fa-calendar-plus mr-2"></i>
                                        Add to Calendar
                                    </a>
                                </div>
                            </div>
                        );
                    })
                ) : (
                    <div className="col-span-full py-16 flex flex-col items-center justify-center bg-[#EFF6FF] rounded-2xl border border-[#DBEAFE] text-center">
                        {/* Calendar illustration */}
                        <div className="relative mb-6 flex items-end justify-center">
                            {/* Left cloud */}
                            <svg className="absolute left-0 bottom-4 w-14 h-10 text-[#BFDBFE] opacity-80" viewBox="0 0 80 50" fill="currentColor">
                                <ellipse cx="40" cy="35" rx="35" ry="15"/>
                                <ellipse cx="25" cy="28" rx="18" ry="14"/>
                                <ellipse cx="55" cy="25" rx="20" ry="16"/>
                            </svg>
                            {/* Right cloud */}
                            <svg className="absolute right-0 bottom-6 w-12 h-8 text-[#BFDBFE] opacity-60" viewBox="0 0 80 50" fill="currentColor">
                                <ellipse cx="40" cy="35" rx="30" ry="13"/>
                                <ellipse cx="28" cy="26" rx="16" ry="13"/>
                                <ellipse cx="52" cy="23" rx="18" ry="14"/>
                            </svg>
                            {/* Calendar icon */}
                            <div className="relative z-10">
                                <svg width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    {/* Calendar body */}
                                    <rect x="8" y="18" width="68" height="60" rx="8" fill="white" stroke="#93C5FD" strokeWidth="2"/>
                                    {/* Calendar header */}
                                    <rect x="8" y="18" width="68" height="20" rx="8" fill="#2563EB"/>
                                    <rect x="8" y="30" width="68" height="8" fill="#2563EB"/>
                                    {/* Hooks */}
                                    <rect x="25" y="10" width="6" height="16" rx="3" fill="#60A5FA"/>
                                    <rect x="59" y="10" width="6" height="16" rx="3" fill="#60A5FA"/>
                                    {/* Grid dots */}
                                    {[0,1,2,3,4,5,6,7,8].map((i) => (
                                        <rect key={i} x={20 + (i % 3) * 18} y={48 + Math.floor(i / 3) * 12} width="10" height="6" rx="2" fill="#BFDBFE"/>
                                    ))}
                                    {/* Live badge */}
                                    <circle cx="68" cy="65" r="14" fill="#2563EB"/>
                                    <text x="68" y="70" textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">((•))</text>
                                </svg>
                            </div>
                            {/* Sparkles */}
                            <span className="absolute top-0 left-8 text-[#93C5FD] text-lg">✦</span>
                            <span className="absolute top-4 right-6 text-[#93C5FD] text-sm">✦</span>
                        </div>

                        <h3 className="text-2xl font-bold text-[#1E1B4B] mb-2">No Live Shows Available</h3>
                        <p className="text-gray-500 max-w-xs text-sm leading-relaxed mb-7">
                            Check back later for upcoming live sessions<br/>and workshops.
                        </p>
                        <button
                            onClick={() => alert("You'll be notified when a live show is scheduled!")}
                            className="flex items-center gap-2 bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-sm font-semibold px-7 py-3 rounded-xl shadow-md shadow-blue-200 transition-all duration-150 hover:scale-[1.02] active:scale-95"
                        >
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                            </svg>
                            Notify Me
                        </button>
                    </div>
                )}
            </div>
        </section>
    );
};

export default LiveShowCard;

if (document.getElementById("live-show")) {
    const Index = ReactDOM.createRoot(document.getElementById("live-show"));
    Index.render(
        <React.StrictMode>
            <FlashMessageProvider>
                <LiveShowCard />
            </FlashMessageProvider>
        </React.StrictMode>
    );
}
