...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    <head>
        ...
        {{-- SEO --}}
        <meta name="description" content="KingsleyKhord is a platform that connects you with the best service providers in your area.">
        <meta name="keywords" content="KingsleyKhord, service providers, local services">
        <meta name="author" content="LegendOSA Consultants">
                        ...
                        <div x-data="{ openMenu: false }" class="flex items-center space-x-4 relative">
                                ...
                                <div class="hidden lg:flex items-center space-x-4">
                                    ...
                                    @php
                                        ...
                                        $recentNotifications = auth()->user() ? auth()->user()->notifications()->latest()->get() : collect();
                                    @endphp
                                    ...
                                    <div x-data="{ openNotif: false }" class="relative">
                                        ...
                                        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                                            ...
                                            <div class="max-h-72 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
                                                ...
                                                @if($recentNotifications->count() > 0)
                                                    @foreach($recentNotifications as $note)
                                                        <div
                                                            ...
                                                            >
                                                                <p class="text-sm font-medium text-gray-800 dark:text-gray-100 leading-snug">
                                                                    <span class="text-pink-600 dark:text-pink-400">
                                                                        {{ $note->data['category'] ?? 'Notification' }}
                                                                    </span>
                                                            ...
                                                            @else
                                                                <span class="mt-1 h-2 w-2 rounded-full bg-gray-300 dark:bg-gray-600"></span>
                                                            @endif
                                ...
                                <div x-data="{ openMobileNotif: false }" class="lg:hidden relative mr-2">
                                    <button @click="openMobileNotif = !openMobileNotif" class="relative text-white focus:outline-none">
                                        <i class="fa fa-bell text-xl"></i>

                                        @if($unreadNotifications > 0)
                                            <span class="absolute -top-1 -right-2 inline-flex items-center justify-center 
                                                px-1.5 py-0.5 text-[10px] font-bold text-white bg-red-500 rounded-full">
                                                {{ $unreadNotifications }}
                                            </span>
                                        @endif
                                    </button>
                                            ...
                                            <div x-show="openMobileNotif"
                                                x-transition
                                                x-cloak
                                                @click.away="openMobileNotif = false"
                                                class="absolute top-8 right-0 w-72 bg-white text-black rounded-lg shadow-2xl p-4 z-[9999]">
                                                ...
                                                <div class="max-h-60 overflow-y-auto divide-y divide-gray-200">
                                                    @if($recentNotifications->count() > 0)
                                                        @foreach($recentNotifications as $note)
                                                            <div class="px-2 py-3 flex items-start justify-between">
                                                                ...
                                                                @if(is_null($note->read_at))
                                                                    <form method="POST" action="{{ route('notifications.markRead', $note->id) }}">
                                                                        @csrf
                                                                        <button class="text-blue-600 text-xs hover:underline ml-2 whitespace-nowrap" type="submit">
                                                                            Mark read
                                                                        </button>
                                                                    </form>
                                                                @endif
                                    ...
                                    <!-- Logout Modal -->
                                    <div x-show="showLogoutModal"  x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
