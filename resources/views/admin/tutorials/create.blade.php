...
<main class="flex-1 p-6 overflow-y-auto">
  ...
  <header class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-800">Upload Tutorial</h2>
    <p class="text-sm text-gray-500">Add a new video tutorial to the community space.</p>
  </header>
  ...
  <div class="bg-white border border-gray-200 rounded-xl shadow-sm max-w-3xl p-6">
    <form action="{{ route('admin.tutorials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          >
            ...
            <option value="iframe" {{ old('video_type') === 'iframe' ? 'selected' : '' }}>Iframe</option>
            <option value="youtube" {{ old('video_type') === 'youtube' ? 'selected' : '' }}>YouTube</option>
            <option value="google" {{ old('video_type') === 'google' ? 'selected' : '' }}>Google Drive</option>
            <option value="local" {{ old('video_type') === 'local' ? 'selected' : '' }}>Local Path</option>
          </select>
        </div>
        ...
        <div>
          ...
          <input 
            ...
            name="author_name" 
            id="author_name" 
            value="{{ old('author_name', 'Kingsley O.') }}"
            class="w-full text-sm px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none"
          >
        </div>
