...
<main class="flex-1 p-6 overflow-y-auto">
  ...
  <header class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-800">Edit Challenge Topic</h2>
    <p class="text-sm text-gray-500">Update this challenge topic's title, description, and pinned video.</p>
  </header>
  ...
  <div class="bg-white border border-gray-200 rounded-xl shadow-sm max-w-3xl p-6">
    <form action="{{ route('admin.student-challenges.update', $challenge->id) }}" method="POST" class="space-y-6">
      @csrf
      @method('PUT')
      ...
      <div class="flex items-center gap-2">
        <input type="checkbox" name="is_pinned" id="is_pinned" value="1" {{ old('is_pinned', $challenge->is_pinned) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-400">
