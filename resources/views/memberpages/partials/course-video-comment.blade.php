<div class="flex gap-3 py-4 first:pt-0" data-comment-id="{{ $comment->id }}">
    ...
    <div class="flex-1 min-w-0">
        ...
        @if ($comment->user_id === auth()->id())
            <div class="comment-edit-form hidden mt-2 space-y-2">
                <textarea class="comment-edit-input w-full border border-gray-300 rounded-md p-2 text-[13.5px] focus:ring-1 focus:ring-gray-900 focus:border-gray-900 transition-colors outline-none resize-none" rows="2">{{ $comment->comment }}</textarea>
        ...
        <div class="comment-replies-list mt-3 space-y-3 border-gray-200 {{ $comment->replies->count() ? 'border-l pl-3' : '' }}">
